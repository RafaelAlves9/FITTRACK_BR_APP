...
export default function WorkoutEditScreen() {
  ...
  const { workouts, updateWorkout, getCustomExercisesByType, addCustomExercise } = useData();
  const { showAlert } = useAlert();

  const workout = workouts.find(w => w.id === id);

  const [step, setStep] = useState<Step>('exercises');
  ...
  useEffect(() => {
    if (workout) {
      const normalizedExercises = Array.isArray(workout.exercises) ? workout.exercises : [];
  ...
  const customExercises = getCustomExercisesByType(workout.type);
  const baseExercises = workout.type === 'musculacao' ? musculationExercises : aerobicExercises;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showAlert('Erro', 'Erro ao atualizar treino');
    } finally {
      setSaving(false);
    }
  };
  ...
  const renderExercisesStep = () => {
    ...
    return (
      <View style={styles.stepContent}>
        <Text style={styles.stepTitle}>Editar Exercícios</Text>
                    ...
                    >
                      <View style={styles.exerciseInfo}>
                        ...
                        <Text style={styles.exerciseCategory}>
                          {exercise.category} • {workout.type === 'musculacao' 
  ...
  const renderNameStep = () => (
    <View style={styles.stepContent}>
      ...
      <Text style={styles.stepSubtitle}>Edite o nome do treino</Text>
