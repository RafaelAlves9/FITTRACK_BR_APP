...
export function useMockAuth(): AuthContextType {
  const context = useMockAuthContext();
    ...
    try {
      ...
      devWarn('[Template:useMockAuth] sendOTP exception:', error);
      return { 
    ...
    try {
      ...
      devWarn('[Template:useMockAuth] verifyOTPAndLogin exception:', error);
      return { 
        error: 'Login failed',
        user: null 
      };
  ...
  const signUpWithPassword = async (email: string, password: string, metadata?: Record<string, any>): Promise<SignUpResult> => {
    ...
    try {
      ...
      devWarn('[Template:useMockAuth] signUpWithPassword exception:', error);
      return { 
  ...
  const signInWithPassword = async (email: string, password: string): Promise<AuthResult> => {
    ...
    try {
      ...
      devWarn('[Template:useMockAuth] signInWithPassword exception:', error);
      return { 
        error: 'Login failed',
        user: null 
      };
  ...
  const logout = async (): Promise<LogoutResult> => {
    ...
    try {
      ...
      if (!result) {
        devWarn('[Template:useMockAuth] Invalid logout result format:', result);
      ...
      devWarn('[Template:useMockAuth] Logout hook exception:', errorMessage);
  ...
  const refreshSession = async () => {
    try {
      ...
      devWarn('[Template:useMockAuth] Refresh session error:', error);
    }
  };
