...
export class MockAuthService {
  ...
  async signInWithPassword(email: string, password: string) {
    ...
    if (options?.password) {
      devLog(`[Template:MockAuth] OTP+Password registration for ${email}, password would be set in real environment`);
    }
    
    if (options?.metadata) {
      devLog(`[Template:MockAuth] User metadata for ${email}:`, options.metadata);
    }
  ...
  onAuthStateChange(callback: (user: AuthUser | null) => void) {
    ...
    const checkAuthState = async () => {
      try {
        ...
        devWarn('[Template:MockAuthService] Error in auth state check:', error);
      }
    };
